...
public static class MalumCheats
{
    ...
    /* Issue with timer, removed
    private static double KickTime = 0;

    public static void LoopKickVentsCheat()
    {
        if (!CheatToggles.loopKickVents) return;

        double currentTime = System.DateTime.Now.TimeOfDay.TotalSeconds;

        if (currentTime >= _nextKickTime)
        {
            _nextKickTime = currentTime + 0.50;
    ...
    */
    ...
    public static void OpenAdminMapCheat()
    {
        var hud = DestroyableSingleton<HudManager>.Instance;
        if (hud != null)
        {
            MapOptions options = new MapOptions();
            options.Mode = MapOptions.Modes.CountOverlay;
        
            hud.ToggleMapVisible(options);
        }
    }
